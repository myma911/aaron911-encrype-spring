...
public class DecryptRequestBodyAdvice implements RequestBodyAdvice {
    ...
    public HttpInputMessage beforeBodyRead(HttpInputMessage inputMessage, MethodParameter parameter, Type targetType, Class<? extends HttpMessageConverter<?>> converterType) throws IOException {
        ...
        String body;
        try {
        	body = IoUtil.read(inputMessage.getBody(), encryptProperty.getCharset());
        }catch (Exception e){
        	log.error("数据读取失败", e);
        ...
        try {
        	InputStream inputStream = IoUtil.toStream(decryptBody, encryptProperty.getCharset());
    ...
    private String switchDecrypt(String formatStringBody, DecryptAnnotationInfoBean infoBean){
        ...
        if(method == DecryptBodyMethod.DES){
            key = CheckUtils.checkAndGetKey(encryptProperty.getAesKey(),key,"DES-KEY");
            return DESEncryptUtil.decrypt(formatStringBody,key);
        }
        if(method == DecryptBodyMethod.AES){
            key = CheckUtils.checkAndGetKey(encryptProperty.getAesKey(),key,"AES-KEY");
